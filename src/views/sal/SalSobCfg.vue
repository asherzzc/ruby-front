  ...
  methods: {
    ...
    hidePop(data) {
      if (this.currentSalary) {
        this.postRequest('/salary/sobcfg/?eid=' + data.id + '&sid=' + this.currentSalary).then(resp => {
          if (resp) {
            this.initEmps()
          }
        });
    ...
    initEmps() {
      this.getRequest("/salary/sobcfg/?start=" + this.currentPage + '&size=' + this.currentSize).then(resp => {
        if (resp) {
          this.emps = resp.data;
